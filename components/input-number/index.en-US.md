...
| addonAfter | The label text displayed after (on the right side of) the input field | slot | - | 4.0 |
| addonBefore | The label text displayed before (on the left side of) the input field | slot | - | 4.0 |
| controls | Whether to show `+-` controls | boolean | true | 4.0 |
| keyboard | If enable keyboard behavior | boolean | true | 4.0 |
| stringMode | Set value as string to support high precision decimals. Will return string value by `change` | boolean | false | 4.0 |
...
| step | The callback function that is triggered when click up or down buttons | (value: number, info: { offset: number, type: 'up' \| 'down' }) => void | 4.0 |

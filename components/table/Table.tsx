  ...
  methods: {
    ...
    setSelectedRowKeys(selectedRowKeys, selectionInfo) {
      ...
      const selectedRows = data.filter((row, i) =>
        selectedRowKeys.includes(this.getRecordKey(row, i)),
      );
      ...
      if (selectWay === 'onSelect' && rowSelection.onSelect) {
        ...
        const changeRows = data.filter((row, i) =>
          changeRowKeys.includes(this.getRecordKey(row, i)),
        );
    ...
    handleFilter(column, nextFilters) {
      ...
      Object.keys(filters).forEach((columnKey) => {
        if (!currentColumnKeys.includes(columnKey)) {
    ...
    renderColumnsDropdown({ prefixCls, dropdownPrefixCls, columns, locale, getPopupContainer }) {
      ...
      return treeMap(columns, (column, i) => {
        ...
        if (column.sorter) {
          ...
          const ascend = sortDirections.includes('ascend') && (
            <CaretUpFilled
          ...
          const descend = sortDirections.includes('descend') && (
            <CaretDownFilled

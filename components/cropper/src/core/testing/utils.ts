import type { AspectRatio, ImageRestriction, SizeRestrictions } from '../typings';
...
export class MockFactory {
  ...
  sizeRestriction({ valid = true, complete = true } = {}) {
    ...
    if (complete || valid ? result.minWidth : this.random() > 0.5) {
      result.maxWidth = valid ? (result?.minWidth||0) + this.random() * MAX_SIZE : this.random() * MAX_SIZE;
    }
    if (complete || valid ? result.minHeight : this.random() > 0.5) {
      result.maxHeight = valid ? (result?.minHeight||0) + this.random() * MAX_SIZE : this.random() * MAX_SIZE;
    }
